...
func testIt(t *testing.T, funcname string, f func(*errgroup.Group, context.Context, io.ReadCloser, []*Checksum) []*Checksum) {
	...
	for checksum := range xchecksum {
		...
		got := f(g, ctx, reader, xchecksum[checksum][:1])
		g.Wait()
...
func testIt2(t *testing.T, funcname string, f func(*errgroup.Group, context.Context, io.ReadCloser, []*Checksum) []*Checksum) {
